...
eps = mypara.eps;
% compute phi and psi
Psi   = myfunc.Psi_vec;      % Nv x 1
Phi_c = myfunc.Phi_c_vec;    % Nv x 1
Phi_n = myfunc.Phi_n_vec;    % Nv x 1
...
for j = 1:Nx
    ...
    if (rho_j > 0) && (m_j < 0)
        ...
        F_star(:, j) = F_eq + theta_j * (Fj - rho_j);
...
%% ===================== update M (was: c) =====================
% Paper model (1.1c):
%   ∂t M - D_M ∂xx M + α M = β ρ
%
% Note:
%   MD in this code approximates (-∂xx) with Neumann BC, i.e.
%       (-∂xx M) ≈ MD * M
%   hence  -D_M ∂xx M  =  D_M * (-∂xx M)  ≈  D_M * MD * M.
%
% Backward Euler in time:
%   (M^{n+1}-M^n)/dt + D_M * MD * M^{n+1} + α M^{n+1} = β ρ^{n+1}
% => (I/dt + D_M*MD + α I) M^{n+1} = M^n/dt + β ρ^{n+1}

DM    = mypara.DM;        % == D_M in paper
alpha = mypara.alpha;     % == α in paper
beta  = mypara.beta;      % == β in paper (请在主程序/参数结构里补上)

MAT_M = speye(Nx)/dt + DM*MD + alpha*speye(Nx);
rhs_M = c'/dt + beta * Rho_CurrentStep';      % use rho^{n+1}
C_CurrentStep = (MAT_M \ rhs_M)';             % here C_CurrentStep stores M^{n+1}


%% ===================== update N (was: n) =====================
% Paper model (1.1c):
%   ∂t N - D_N ∂xx N + γ ρ N = 0
%
% Using MD ≈ (-∂xx):
%   (N^{n+1}-N^n)/dt + D_N*MD*N^{n+1} + γ ρ^{n+1} N^{n+1} = 0
% => (I/dt + D_N*MD + γ*diag(ρ^{n+1})) N^{n+1} = N^n/dt

DN    = mypara.DN;        % == D_N in paper (请在参数结构里补上)
gamma = mypara.gamma;     % == γ in paper (请在参数结构里补上)

MAT_N = speye(Nx)/dt + DN*MD + gamma * spdiags(Rho_CurrentStep', 0, Nx, Nx);
rhs_N = n'/dt;
N_CurrentStep = (MAT_N \ rhs_N)';             % here N_CurrentStep stores N^{n+1}